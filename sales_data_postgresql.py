...
# Income Statement (query diubah menjadi PostgreSQL)
income_statement = pd.read_sql_query(
    """
        ...
        SELECT
            Product, Quantity, Revenue, Profit, ROUND(100.0*Profit/Revenue, 2) AS GPM
        FROM new_data;
    """, conn
)
...
penjualan_tiap_periode = pd.read_sql_query(
    """
        SELECT
            TO_CHAR(dus.day_date, 'YYYY-MM') AS Periode,
            SUM(dus.units_sold) AS Quantity
...
top_sales_performance = pd.read_sql_query(
    """
        WITH new_data AS (
            SELECT
                TO_CHAR(day_date, 'YYYY-MM') AS Periode,
